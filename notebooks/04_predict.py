...
def load_best_model_from_runs(model_name: str, mlflow_client: MlflowClient):
    ...
    try:
        ...
        if not best_run:
            logger.info(
                f"No production run found for {model_name}, searching for best historic run."
            )
            runs = mlflow_client.search_runs(
                experiment_ids=exp_ids,
        ...
        logger.info(
            f"Loaded {model_name} from Run ID {run_id} (MAPE: {best_run.data.metrics.get('mape'):.4f})"
        )
...
def prepare_inference_features(
    spark, config: dict, horizon_hours: int, forecast_run_at: datetime
) -> pd.DataFrame:
    ...
    for t in future_ts:
        ...
        temp_c = (
            temp_matches.iloc[0] if not temp_matches.empty else history_pd["temperature_c"].iloc[-1]
        )
        hum_matches = history_pd[history_pd["timestamp"] == proxy_time]["humidity_pct"]
        humidity_pct = (
            hum_matches.iloc[0] if not hum_matches.empty else history_pd["humidity_pct"].iloc[-1]
        )
        cc_matches = history_pd[history_pd["timestamp"] == proxy_time]["cloud_cover_pct"]
        cloud_cover_pct = (
            cc_matches.iloc[0] if not cc_matches.empty else history_pd["cloud_cover_pct"].iloc[-1]
        )
...
for h in horizons:
    ...
    logger.info(
        f"Horizon {h}h: wrote {len(forecasts_df)} forecast rows using {actual_model_name} v{ver}"
    )
