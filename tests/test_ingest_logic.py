...
def get_hourly_targets(fetch_start: datetime, fetch_end: datetime):
    """Generate hourly (start, filename) pairs for backfill segment logic."""
    targets = []
    current = fetch_start
    while current < fetch_end:
        filename = current.strftime("%Y-%m-%dT%H-00-00Z.json")
        targets.append((current, filename))
        current += timedelta(hours=1)
    return targets


def test_lookback_hours_24_produces_24_hourly_files():
    """AC6: With lookback_hours=24, the fetch window produces exactly 24 hourly target timestamps."""
    run_dt = datetime(2026, 7, 26, 12, 0, 0)
    lookback = 24
    fetch_start = run_dt - timedelta(hours=lookback)
    fetch_end = run_dt

    targets = get_hourly_targets(fetch_start, fetch_end)

    assert len(targets) == 24
    assert targets[0][0] == datetime(2026, 7, 25, 12, 0, 0)
    assert targets[-1][0] == datetime(2026, 7, 26, 11, 0, 0)
    assert targets[0][1] == "2026-07-25T12-00-00Z.json"
    assert targets[-1][1] == "2026-07-26T11-00-00Z.json"
